...
void joyDrvDInputSetCooperativeLevel(int port) {
	...
	res = IDirectInputDevice8_SetCooperativeLevel(joy_drv_lpDID[port],
		      gfx_drv_hwnd,
...
void joyDrvDInputAcquire(int port) {
  ...
  if (joy_drv_in_use) {
    ...
    joyDrvDInputSetCooperativeLevel( port );
	  ...
	  if ((res = IDirectInputDevice8_Acquire( joy_drv_lpDID[port] )) != DI_OK)
...
BOOL FAR PASCAL joyDrvInitJoystickInput(LPCDIDEVICEINSTANCE pdinst, 
                                  LPVOID pvRef) 
{ 
