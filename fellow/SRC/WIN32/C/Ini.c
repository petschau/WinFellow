...
bool iniGetGfxDebugImmediateRendering(ini *initdata)
{
  return initdata->m_gfxDebugImmediateRendering;
}

void iniSetGfxDebugImmediateRendering(ini *initdata, bool gfxDebugImmediateRendering)
{
  initdata->m_gfxDebugImmediateRendering = gfxDebugImmediateRendering;
}
...
void iniSetDefaults(ini *initdata)
{
  ...
  Service->Fileops.GetDefaultConfigFileName(ini_default_config_filename);
  ...
  iniSetGfxDebugImmediateRendering(initdata, false);
}
...
BOOLE iniSetOption(ini *initdata, STR *initoptionstr)
{
  ...
  if (result)
  {
    ...
    *value++ = '\0';
    ...
    {
      iniSetPauseEmulationWhenWindowLosesFocus(initdata, stricmp(value, "true") == 0);
    }
    else if (stricmp(option, "gfx_debug_immediate_rendering") == 0)
    {
      iniSetGfxDebugImmediateRendering(initdata, stricmp(value, "true") == 0);
    }
    else
      result = FALSE;
  }
...
BOOLE iniSaveOptions(ini *initdata, FILE *inifile)
{
  ...
  fprintf(inifile, "gfx_debug_immediate_rendering=%s\n", iniGetGfxDebugImmediateRendering(initdata) ? "true" : "false");
...
void iniManagerStartup(iniManager *initdatamanager)
{
  ...
  Service->Fileops.GetGenericFileName(ini_filename, "WinFellow", INI_FILENAME);
