...
{
  if ((lp = strchr(lp, sep)) == NULL) return -1;
  if (sscanf(lp + 1, "%s", str) != 1) return -1;
  return 0;
}
...
{
  uint32 out = htonl(v);
  if (fwrite(&out, sizeof(out), 1, fp) != 1) return -1;
  return 0;
}
...
template <int N, int I = N> struct vm_read_memory
{
...
template <int N> struct vm_read_memory<N, 1>
{
  static inline uint32 apply(uint8 *addr)
  {
    return addr[0];
...
template <int N, int I = N> struct vm_write_memory
{
  static inline void apply(uint8 *addr, uint32 value)
  {
    addr[I - 1] = value;
    vm_write_memory<N, I - 1>::apply(addr, value >> 8);
  }
};

template <int N> struct vm_write_memory<N, 1>
{
  static inline void apply(uint8 *addr, uint32 value)
  {
    addr[0] = value;
...
static int read_testcase_text(FILE *fp, m68k_testcase_t *tp)
{
  ...
  if (feof(fp)) return PARSER_EOF;
  if (ferror(fp)) return PARSER_ERROR;
  ...
  while (fgets(line, sizeof(line), fp) != NULL)
  {
    ...
    {
      if (inst->name[0] == '\0') error |= get_str(inst->name, &line[11], ':');
    }
    else if (strncmp(line, "broken opcode", 13) == 0)
    {
      if (inst->name[0] == '\0') error |= get_str(inst->name, &line[13], ':');
    }
...
template <int FB, int FE> struct static_mask
{
  enum
  {
    value = (0xffffffff >> FB) ^ (0xffffffff >> (FE + 1))
  };
};

template <int FB> struct static_mask<FB, 31>
{
  enum
  {
    value = 0xffffffff >> FB
...
template <int FB, int FE> struct bit_field
{
...
static int get_register_binary(FILE *fp, m68k_cpu_state_t *csp, int type, int regno)
{
  ...
  switch (type)
  {
    ...
    case BF_CPU_REG_ADDR:
      ...
      break;
    case BF_CPU_REG_FP: error = -1; break;
  }
  return 0;
}
...
{
  int error = 0;
  if (CPU_reg1_type::test(value)) error |= get_register_binary(fp, csp, CPU_reg1_type::extract(value), CPU_reg1_id::extract(value));
  if (CPU_reg2_type::test(value)) error |= get_register_binary(fp, csp, CPU_reg2_type::extract(value), CPU_reg2_id::extract(value));
  if (CPU_reg3_type::test(value)) error |= get_register_binary(fp, csp, CPU_reg3_type::extract(value), CPU_reg3_id::extract(value));
  if (CPU_reg4_type::test(value)) error |= get_register_binary(fp, csp, CPU_reg4_type::extract(value), CPU_reg4_id::extract(value));
  if (CPU_reg5_type::test(value)) error |= get_register_binary(fp, csp, CPU_reg5_type::extract(value), CPU_reg5_id::extract(value));
  return error;
}
  ...
  while (get_be32(fp, &value) == 0)
  {
    switch (BLK_signature::extract(value))
    {
      case BF_SIGNATURE_END:
        ...
        {
          case BF_END_TEST: return rc | PARSER_READ_TEST;
          case BF_END_SERIES: return PARSER_READ_SERIES;
        }
      ...
      case BF_SIGNATURE_INSTRUCTION:
        ...
        if ((inst = (m68k_instruction_t *)malloc(sizeof(*inst) + (2 * len))) == NULL) return PARSER_ERROR;
        tp->inst = inst;
      ...
      case BF_SIGNATURE_CPU_STATE:
        switch (++n_cpu_blocks)
        {
          case 1: csp = &tp->input_state; break;
          case 2: csp = &tp->output_state; break;
          case 3: csp = &tp->gen_state; break;
          default: fprintf(stderr, "ERROR: too many CPU blocks\n"); return PARSER_ERROR;
        }
      ...
      case BF_SIGNATURE_RESULTS_STATS:
        ...
        {
          case BF_RES_N_TESTS: gen_tests = RES_count::extract(value); break;
        ...
        break;
      default: fprintf(stderr, "ERROR: unhandled block signature %d\n", BLK_signature::extract(value)); return PARSER_ERROR;
    }
  }
...
static int check_test(m68k_testcase_t *tp)
{
  ...
  if (tp->gen_state.dregs[0] != tp->output_state.dregs[0]) error |= OPCODE_DELTA_D0;
  if (tp->gen_state.dregs[1] != tp->output_state.dregs[1]) error |= OPCODE_DELTA_D1;
  if (tp->gen_state.ccr != tp->output_state.ccr) error |= OPCODE_DELTA_FLAGS;
  return error;
}
...
static int get_mnemo(m68k_instruction_t *inst)
{
  ...
  static const struct
  {
    ...
    uint16 match;
    int insn;
  } instr_table[] = {{0xf118, 0xe108, i_LSL}, {0xf118, 0xe008, i_LSR}, {0xf118, 0xe100, i_ASL}, {0xf118, 0xe000, i_ASR}, {0x0000, 0x0000, i_UNKNOWN}};
  int insn = i_UNKNOWN;
  for (int i = 0; instr_table[i].insn != i_UNKNOWN; i++)
  {
    if ((opcode & instr_table[i].mask) == instr_table[i].match) return instr_table[i].insn;
  }
  return i_UNKNOWN;
}
...
static void patch_immi(m68k_cpu *cpu, m68k_testcase_t *tp)
{
  ...
  switch (inst->mnemo)
  {
    ...
    case i_LSR:
      // match LSd.z D0,D1
      if ((opcode & 0xfe3f) == 0xe029) goto do_patch_shift;
    case i_ASL:
    case i_ASR:
      // match ASd.z D0,D1
      if ((opcode & 0xfe3f) == 0xe021) goto do_patch_shift;
      break;
    do_patch_shift:
      if (d0 >= 1 && d0 <= 8)
      {
...
{
#if ENABLE_PATCHES
  if (patches & PATCH_IMMI) patch_immi(cpu, tp);
#endif
}
...
static void print_instruction(m68k_instruction_t *inst, int format = FORMAT_DEFAULT)
{
  switch (format)
  {
    case FORMAT_TEXT: print_instruction_text(inst); break;
    case FORMAT_BINARY: print_instruction_binary(inst); break;
    default: fprintf(stderr, "ERROR: unhandled format %d\n", format); abort();
  }
}
...
static void print_test_text(m68k_testcase_t *tp)
{
  ...
  printf("before d0=%08x    d1=%08x    CCR=%s (%d)\n", tp->input_state.dregs[0], tp->input_state.dregs[1], ccr2str(tp->input_state.ccr), tp->input_state.ccr);
  printf(
      "M68K   d0=%08x    d1=%08x    CCR=%s (%d) %s\n",
      tp->output_state.dregs[0],
      tp->output_state.dregs[1],
      ccr2str(tp->output_state.ccr),
      tp->output_state.ccr,
      ((tp->input_state.ccr ^ tp->output_state.ccr) & (M68K_CCR_N | M68K_CCR_V)) ? "NV_changed" : "NV_same");
  printf(
      "GEN    d0=%08x %c  d1=%08x %c  CCR=%s %c (%d)\n",
      tp->gen_state.dregs[0],
      (error & OPCODE_DELTA_D0) ? '*' : ' ',
      tp->gen_state.dregs[1],
      (error & OPCODE_DELTA_D1) ? '*' : ' ',
      ccr2str(tp->gen_state.ccr),
...
static void print_test(m68k_testcase_t *tp, int format = FORMAT_DEFAULT)
{
  switch (format)
  {
    case FORMAT_TEXT: print_test_text(tp); break;
    case FORMAT_BINARY: print_test_binary(tp); break;
    default: fprintf(stderr, "ERROR: unhandled format %d\n", format); abort();
  }
}
...
{
  printf("%d errors out of %d tests done for %s (%04x)\n", n_errors, n_tests, tp->inst->name, tp->inst->words[0]);
}
...
static void print_test_end(m68k_testcase_t *tp, int n_errors, int n_tests, int format = FORMAT_DEFAULT)
{
  switch (format)
  {
    case FORMAT_TEXT: print_test_end_text(tp, n_errors, n_tests); break;
    case FORMAT_BINARY: print_test_end_binary(tp, n_errors, n_tests); break;
    default: fprintf(stderr, "ERROR: unhandled format %d\n", format); abort();
  }
}
...
int main(int argc, char *argv[])
{
    ...
    else if (strcmp(arg, "--patches") == 0)
    {
      if (++i < argc)
      {
        ...
        for (;;)
        {
          ...
          {
            int len = cp - arg;
            if (strncmp(arg, "immi", len) == 0) patches |= PATCH_IMMI;
            arg = cp + 1;
            if (*cp == '\0') break;
          }
          ++cp;
  ...
  if (cmd == CMD_CONVERT || !verbose) stdout_off(&stdout_state);
  ...
  while (!feof(stdin) && !ferror(stdin))
  {
    ...
    switch (input_format)
    {
      case FORMAT_TEXT:
        ...
        if (rc != PARSER_SW_BINARY) break;
        input_format = FORMAT_BINARY;
        // fall-through
      case FORMAT_BINARY: rc = read_testcase_binary(stdin, &testcase); break;
    }
      ...
      if (rc & PARSER_READ_OPCODE)
      {
        ...
        switch (cmd)
        {
          case CMD_TEST:
            ...
            if (!verbose) break;
            // fall-through
          case CMD_PRINT: print_instruction(testcase.inst, FORMAT_TEXT); break;
          case CMD_CONVERT: print_instruction(testcase.inst, output_format); break;
        }
      }
      if (rc & PARSER_READ_TEST)
      {
        ...
        switch (cmd)
        {
          case CMD_TEST:
            ...
            if (!error && !verbose) break;
            // fall-through
          case CMD_PRINT: print_test(&testcase, FORMAT_TEXT); break;
          case CMD_CONVERT: print_test(&testcase, output_format); break;
        }
        if (error) ++n_errors;
      }
    }
    ...
    else if (rc == PARSER_READ_SERIES)
    {
      switch (cmd)
      {
        ...
        case CMD_PRINT:
          ...
          break;
        case CMD_CONVERT: print_test_end(&testcase, n_errors, n_tests, output_format);
      }
  ...
  if (testcase.inst) free(testcase.inst);
  delete cpu;
