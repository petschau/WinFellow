...
{
  uint32_t y = graphAdjustVPosY(_core.Clocks->GetChipTime().Line, _core.Clocks->GetChipTime().Cycle);
...
{
  uint32_t x = _core.Clocks->GetChipTime().Cycle;
  uint32_t y = graphAdjustVPosY(_core.Clocks->GetChipTime().Line, x);
...
void wdiwstrt(uint16_t data, uint32_t address)
{
  ...
  {
    GraphicsContext.Commit(_core.Clocks->GetChipTime().Line, _core.Clocks->GetChipTime().Cycle);
...
{
  uint32_t currentY = _core.Clocks->GetChipTime().Line;
