...
{
  uint32_t currentX = _clocks.GetChipTime().Cycle;
  if (currentX >= 18)
  {
  ...
  (*item)->raster_y = _clocks.GetChipTime().Line;
}
  ...
  if (output_sprite_log == TRUE)
  {
    *((uint16_t *)((uint8_t *)sprpt_debug + sprite_number * 4 + 2)) = (uint16_t)data & 0x01f;
    sprintf(
        ...
        _clocks.GetChipTime().Line,
        2 * (_clocks.GetChipTime().Cycle - 16),
        sprite_number,
...
void LineExactSprites::NotifySprdataChanged(uint16_t data, unsigned int sprite_number)
{
  ...
  if (output_sprite_log == TRUE)
  {
    sprintf(buffer, "(y, x) = (%u, %u): call to spr%udata\n", _clocks.GetChipTime().Line, 2 * (_clocks.GetChipTime().Cycle - 16), sprite_number);
    _core.Log->AddLog2(buffer);
  }
}
...
void LineExactSprites::DMASpriteHandler()
{
  ...
  uint32_t i, count;
  uint32_t currentY = _clocks.GetChipTime().Line;
