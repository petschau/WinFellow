...
void wdbgUpdateSpriteState(HWND hwndDlg)
{
  ...
  if (hDC != NULL)
  {
    ...
    sprintf(
      s, "Spr0pt-%.6X Spr1pt-%.6X Spr2pt-%.6X Spr3pt - %.6X", sprite_registers.sprpt[0], sprite_registers.sprpt[1], sprite_registers.sprpt[2], sprite_registers.sprpt[3]);
    y = wdbgLineOut(hDC, s, x, y);

    sprintf(
      s, "Spr4pt-%.6X Spr5pt-%.6X Spr6pt-%.6X Spr7pt - %.6X", sprite_registers.sprpt[4], sprite_registers.sprpt[5], sprite_registers.sprpt[6], sprite_registers.sprpt[7]);
    y = wdbgLineOut(hDC, s, x, y);
...
void wdbgUpdateScreenState(HWND hwndDlg)
{
  ...
  if (hDC != NULL)
  {
    ...
    sprintf(s, "DIWSTRT - %.4X DIWSTOP - %.4X DDFSTRT - %.4X DDFSTOP - %.4X LOF     - %.4X", diwstrt, diwstop, ddfstrt, ddfstop, lof);
    y = wdbgLineOut(hDC, s, x, y);

    sprintf(s, "BPLCON0 - %.4X BPLCON1 - %.4X BPLCON2 - %.4X BPL1MOD - %.4X BPL2MOD - %.4X", _core.Registers.BplCon0, bplcon1, _core.Registers.BplCon2, bpl1mod, bpl2mod);
    y = wdbgLineOut(hDC, s, x, y);
...
void wdbgUpdateSoundState(HWND hwndDlg)
{
  ...
  if (hDC != NULL)
  {
    ...
    sprintf(s, "A0: %X A1: %X A2: %X A3: %X A5: %X", soundState0, soundState1, soundState2, soundState3, soundState5);
    y = wdbgLineOut(hDC, s, x, y);
...
void wdebInitInstructionColumns()
{
  ...
  lv_col.pszText = (LPSTR)"Address";
  lv_col.cx = 64;
  lv_col.iSubItem = 0;
  ListView_InsertColumn(hList, 0, &lv_col);

  lv_col.pszText = (LPSTR)"Opcode data";
  lv_col.cx = 128;
  lv_col.iSubItem = 1;
  ListView_InsertColumn(hList, 1, &lv_col);

  lv_col.pszText = (LPSTR)"Instruction";
  lv_col.cx = 64;
  lv_col.iSubItem = 2;
  ListView_InsertColumn(hList, 2, &lv_col);

  lv_col.pszText = (LPSTR)"Operands";
  lv_col.cx = 128;
  lv_col.iSubItem = 3;
  ...
  {
    lv_item.pszText = (LPSTR)"x";
    lv_item.iItem = i;
    lv_item.iSubItem = 0;
    ListView_InsertItem(hList, &lv_item);

    lv_item.pszText = (LPSTR)"y";
    lv_item.iItem = i;
    lv_item.iSubItem = 1;
    ListView_SetItem(hList, &lv_item);

    lv_item.pszText = (LPSTR)"z";
    lv_item.iItem = i;
    lv_item.iSubItem = 2;
    ListView_SetItem(hList, &lv_item);
  }
...
const char* wdbg_registernames[] = { "D0", "D1", "D2", "D3", "D4", "D5", "D6", "D7", "A0", "A1", "A2", "A3", "A4", "A5", "A6", "A7", "PC", "USP", "SSP", "MSP", "ISP", "VBR", "SR" };
...
void wdebInitRegisterColumns()
{
  ...
  lv_col.pszText = (LPSTR)"Register";
  lv_col.cx = 64;
  lv_col.iSubItem = 0;
  ListView_InsertColumn(hList, 0, &lv_col);

  lv_col.pszText = (LPSTR)"Value";
  lv_col.cx = 64;
  lv_col.iSubItem = 1;
  ListView_InsertColumn(hList, 1, &lv_col);
  ...
  {
    lv_item.pszText = (LPSTR)wdbg_registernames[i];
    lv_item.iItem = i;
    lv_item.iSubItem = 0;
    ListView_InsertItem(hList, &lv_item);

    lv_item.pszText = (LPSTR)"";
    lv_item.iItem = i;
    lv_item.iSubItem = 1;
    ListView_SetItem(hList, &lv_item);
  }
...
INT_PTR CALLBACK wdebDebuggerDialogProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
  ...
  case WM_PAINT: wdebUpdateCpuDisplay(); break;
  ...
  case WM_COMMAND:
    if (HIWORD(wParam) == BN_CLICKED)
    {
      ...
      case IDC_BTN_BREAK:
        ...
        break;
      case IDOK:
      case IDCANCEL: wdeb_action = WDEB_EXIT; break;
      default: break;
      }
    }
    break;
