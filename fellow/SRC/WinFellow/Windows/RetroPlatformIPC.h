...
#define RP_FEATURE_SCALING_SUBPIXEL                                                                                                                                           \
  0x00002000 // supports sub-pixel scaling of windowed and fullscreen modes (i.e. not just integer multipliers like 1X, 2X, etc., but stretch to fill any desired pixel size)
...
typedef struct RPScreenMode
{
  ...
  LONG lClipLeft;     // in guest pixel units (Amiga: Super Hires or RTG); -1 = ignore (0 is a valid value); guest should also ignore this when receiving data if
                      // RP_CLIPFLAGS_AUTOCLIP or RP_CLIPFLAGS_NOCLIP; see http://www.retroplatform.com/kb/19-115
  LONG lClipTop;      // in guest pixel units (Amiga: interlaced or RTG); -1 = ignore (0 is a valid value); guest should also ignore this when receiving data if
                      // RP_CLIPFLAGS_AUTOCLIP or RP_CLIPFLAGS_NOCLIP
  LONG lClipWidth;    // in guest pixel units (Amiga: Super Hires or RTG); -1 = ignore; guest should also ignore this when receiving data if RP_CLIPFLAGS_AUTOCLIP or
                      // RP_CLIPFLAGS_NOCLIP
  LONG lClipHeight;   // in guest pixel units (Amiga: interlaced or RTG); -1 = ignore; guest should also ignore this when receiving data if RP_CLIPFLAGS_AUTOCLIP or
                      // RP_CLIPFLAGS_NOCLIP
  HWND hGuestWindow;  // only valid for RP_IPC_TO_HOST_SCREENMODE
  DWORD dwClipFlags;  // clip flags (or 0)
  LONG lTargetWidth;  // in exact host pixels; if set, must also set lTargetHeight; ignored unless RP_SCREENMODE_SCALE_TARGET is set (resulting size is result of clipping and
                      // scaling); RP_SCREENMODE_SCALING_SUBPIXEL and RP_SCREENMODE_SCALING_STRETCH are taken into account
...
#define RP_SCREENMODE_SCALE_TARGET                                                                                                                                            \
  0x000000FE // scale to maximum within lTargetWidth/lTargetHeight (default: maximum possible integer multiplication, no subpixel stretching, preserve ratio; may change to
             // non-integer mode depending on RP_SCREENMODE_SCALING_SUBPIXEL and RP_SCREENMODE_SCALING_STRETCH)
#define RP_SCREENMODE_SCALE_MAX                                                                                                                                               \
  0x000000FF // scale to auto-determined maximum size (default: maximum possible nX integer multiplication, no subpixel stretching, preserve ratio; may change to non-integer
             // mode depending on RP_SCREENMODE_SCALING_SUBPIXEL and RP_SCREENMODE_SCALING_STRETCH); must be set in fullscreen (shared or exclusive) modes
...
#define RP_SCREENMODE_DISPLAY_FULLSCREEN_1                                                                                                                                    \
  0x00000100 // fullscreen on primary (default) display; requires RP_SCREENMODE_SCALE_MAX and RP_SCREENMODE_FULLSCREEN_EXCLUSIVE or RP_SCREENMODE_FULLSCREEN_SHARED
#define RP_SCREENMODE_DISPLAY_FULLSCREEN_2                                                                                                                                    \
  0x00000200 // fullscreen on secondary display (fallback to 1 if unavailable); requires RP_SCREENMODE_SCALE_MAX and RP_SCREENMODE_FULLSCREEN_EXCLUSIVE or
             // RP_SCREENMODE_FULLSCREEN_SHARED
...
#define RP_SCREENMODE_FULLSCREEN_EXCLUSIVE                                                                                                                                    \
  0x00000000 // use display card video modes (aka "real fullscreen", may change resolution/frequency); must have RP_SCREENMODE_DISPLAY_FULLSCREEN_n and RP_SCREENMODE_SCALE_MAX
             // set
#define RP_SCREENMODE_FULLSCREEN_SHARED                                                                                                                                       \
  0x00010000 // use "full window" (desktop size) when in fullscreen (no gfx card fullscreen); must have RP_SCREENMODE_DISPLAY_FULLSCREEN_n and RP_SCREENMODE_SCALE_MAX set
...
#define RP_SCREENMODE_SCALING_SUBPIXEL                                                                                                                                        \
  0x00100000 // use sub-pixel (non-integer) scaling in RP_SCREENMODE_SCALE_TARGET or RP_SCREENMODE_SCALE_MAX modes; if not set, up to four black bars may be added; if set, up
             // to two black bars may be added
#define RP_SCREENMODE_SCALING_STRETCH                                                                                                                                         \
  0x00200000 // "stretch to fill" (do not preserve original ratio) in RP_SCREENMODE_SCALE_TARGET or RP_SCREENMODE_SCALE_MAX modes; if set, no black bars are added
#define RP_SCREENMODE_PIXEL_ORIGINAL_RATIO                                                                                                                                    \
  0x00400000 // use pixel original ratio (when not set, square pixel ratio or a multiple thereof is used, which grants optimal sharpness and avoids screen distortions)
...
#define RP_CLIPFLAGS_NOCLIP                                                                                                                                                   \
  0x00000002 // ignore all 4 Clip values (same as all values = -1) and use 0:0 offset and maximum possible size (probably ugly, but good for adjusting clip area manually)

//
// Integer vs. subpixel scaling, and stretching with or without original ratio
//
// By default, the guest is only expected to be able to scale (resize, units being guest to host pixels) the emulated screen by an integer number of times, e.g. 1X, 2X, 3X,
// etc., as indicated in RP_FEATURE_SCREEN... This means that when going to fullscreen mode (RP_SCREENMODE_SCALE_MAX) or to a window mode with lTargetWidth and lTargetHeight
// set (RP_SCREENMODE_SCALE_TARGET), the content will be an integer number of times the original number of pixels, and surrounded by black bars if necessary. Integer scaling
// is thus the default behavior, and it can be modified by setting RP_SCREENMODE_SCALING_SUBPIXEL and RP_SCREENMODE_SCALING_STRETCH.
//
// In a known-size target (e.g. fullscreen), unless RP_SCREENMODE_SCALING_SUBPIXEL or RP_SCREENMODE_SCALING_STRETCH are set, there may be up to four black bars to fill any
// unused space.
//
// RP_SCREENMODE_SCALING_SUBPIXEL (used only if RP_FEATURE_SCALING_SUBPIXEL was set) allows for subpixel resize scenarios beyond integer 1X, 2X, etc. This is both in
// fullscreen and in window mode.
//
// In a known-size target (e.g. fullscreen), RP_SCREENMODE_SCALING_SUBPIXEL allows for "stretch to fill" with black areas only on two sides out of four (e.g. 4:3 displayed on
// 16:9 will fill to the top and bottom, and have bars only on the left and right).
//
// RP_SCREENMODE_SCALING_STRETCH (used only if RP_FEATURE_SCALING_STRETCH was set) allows to break (not respect) the original screen ratio and allows for maximum "stretch to
// fill" with no black areas. It is used with RP_SCREENMODE_SCALING_SUBPIXEL.
//
// In theory RP_SCREENMODE_SCALING_STRETCH could be used alone (without RP_SCREENMODE_SCALING_SUBPIXEL), but it probably doesn't make practical sense. In any case, the
// theoretical result would be scaling with different integer values for X and Y.
//
//
// Typical scenarios:
//
...
//
// There is a (theoretical) fourth case (RP_SCREENMODE_SCALING_STRETCH without RP_SCREENMODE_SCALING_SUBPIXEL), which could result in different integer multiplications along
// each axis. The player will not request such a mode.
//
//
// Clipping Examples
//
...
// In RP_SCREENMODE_FULLSCREEN_SHARED mode, the net clipped region will be scaled to the maximum possible (integer scaling factor or otherwise, if
// RP_SCREENMODE_SCALING_SUBPIXEL and RP_SCREENMODE_SCALING_STRETCH are set), centered and padded by black bars if necessary (unless RP_SCREENMODE_SCALING_SUBPIXEL and
// RP_SCREENMODE_SCALING_STRETCH are set). In RP_SCREENMODE_FULLSCREEN_EXCLUSIVE mode ("hardware fullscreen"), the net clipped region will be centered in the smallest possible
// compatible hardware mode (in consideration of RP_SCREENMODE_USETVM) and padded by a black border. If an Amiga application sets a different Amiga chipset screen mode, the
// "container" window size will remain unchanged (because it is still constrained by the clipping values). If an Amiga application sets a different RTG screen mode, the
// "container" window size will reflect the new RTG size (instead of the Amiga clip size) and apply RP_SCREENMODE_SCALE_nX.
//
...
// If an Amiga application sets a different Amiga or RTG chipset screen mode, the "container" window size may change (because it is not constrained by any host-set clipping
// values), in consideration of RP_SCREENMODE_SCALE_nX.
//
//
// Notes
//
...
//
// If the user wants to adjust clipping, or for automated grabs and calculations, it is possible to set RP_CLIPFLAGS_NOCLIP, which will widen the window to the maximum (within
// lTargetWidth+lTargetHeight/fullscreen constraints).
//
// Both in fullscreen and in window mode, whenever the guest sets or changes the net window size or the scaling factor (initially, or due to a command it receives, or due to
// Amiga-sourced changes), it sends an RPScreenMode update to the host. This is useful also in fullscreen mode, as it allows to display the appropriate scaling options to
// return in window mode.

//
// In window mode, if no lTargetWidth and lTargetHeight are set, when the host asks for a change in clipping (net content size), the host window size will be adjusted taking
// into account the current integer multiplication factor.
//
// In window mode, after a change of clipping or size, the player may have to reset the visual hilight of the 1X, 2X etc. buttons according to new RP_IPC_TO_HOST_SCREENMODE
...
typedef struct RPInputDeviceDescription
{
  ...
  WCHAR szHostInputID[260];   // host device "ProductGUID InstanceGUID", GUID, etc. (can be any format, as long as the string is unique across all devices; keyboard layout
                              // strings must not contain spaces)
  WCHAR szHostInputName[260]; // host device product description string ("5-Axis,12-Button with POV", "HID keyboard device", etc.) as listed by Windows; identical devices will
                              // result in identical strings (it is up to the host to add " (2)" etc. to the display names)
  ...
  DWORD dwInputDeviceFeatures; // supported guest-side usage bitfield (uses feature flags, e.g. RP_FEATURE_INPUTDEVICE_MOUSE | RP_FEATURE_INPUTDEVICE_JOYSTICK, etc.); example:
                               // PC analog joystick can be used as Amiga mouse and joystick
...
typedef struct RPDeviceContent
{
  ...
  WCHAR szContent[260];  // if RP_DEVICECATEGORY_INPUTPORT (mice only), then host device ID, otherwise full path and name of the media image file to load, if file content (not
                         // used for input devices, which only use szHostInputID); szContent is ignored if btDeviceCategory == RP_DEVICECATEGORY_INPUTPORT and dwInputDevice ==
                         // RP_INPUTDEVICE_EMPTY
} RPDEVICECONTENT;
...
#define RP_HOSTINPUT_KEYBOARD                                                                                                                                                 \
  8 // [LEGACY] Keyboard Layout, using DirectInput keyboard scan codes (e.g. "KeyboardJoystick Left=0x4B Right=0x4D Up=0x48 Down=0x50 Fire=0x4C Autofire=0x38 Fire2=0x52
    // Rewind=0xB5 Play=0x37 FastForward=0x4A Green=0x47 Yellow=0x49 Red=0x4F Blue=0x51" set in szContent); introduced in RP API 3.3 to replace other keyboard layout modes
...
#define RP_INPUTDEVICE_EMPTY                                                                                                                                                  \
  0 // no device (empty port); can also be used from guest to host when something fails, to show to user that port is empty and requires new selection
...
#define RP_INPUTDEVICE_ANALOGSTICK                                                                                                                                            \
  6 // [CURRENTLY UNUSED] Analog joystick; on Amiga: 2nd and 3rd button lines used as analog X/Y axes, digital directions used as buttons 1-4, plus original fire
...
// Device Read/Write status (used in RP_IPC_TO_HOST_DEVICECONTENT, RP_IPC_TO_GUEST_DEVICECONTENT; used for device categories RP_DEVICECATEGORY_FLOPPY, RP_DEVICECATEGORY_HD,
...
#define RP_DEVICEFLAGS_MOUSE_ABSOLUTE                                                                                                                                         \
  0x00000002 // Mouse features on guest side, but absolute-only device on host side (virtual/remote machine, limited tablet driver, etc.); WinUAE: "mousehack" mode
...
// Device Read/Write status (used in RP_IPC_TO_HOST_DEVICEREADWRITE, RP_IPC_TO_GUEST_DEVICEREADWRITE; used for device categories RP_DEVICECATEGORY_FLOPPY,
// RP_DEVICECATEGORY_HD, RP_DEVICECATEGORY_TAPE, RP_DEVICECATEGORY_CARTRIDGE)
...
typedef struct RPScreenCapture
{
  ...
  WCHAR szScreenFiltered[260]; // "\0" or full path and name (Unicode) of the BMP file to save - filtered (e.g. scanline effects), scaled (e.g. line-doubled and
                               // 2X-multiplied), clipped (overscan cropped) bitmap (i.e. as displayed in host environment)
} RPSCREENCAPTURE;
...
#define RP_GUESTSCREENFLAGS_MODE(m)                                                                                                                                           \
  ((m)&RP_GUESTSCREENFLAGS_MODEMASK) // returns RP_GUESTSCREENFLAGS_MODE_DIGITAL, RP_GUESTSCREENFLAGS_MODE_NTSC or RP_GUESTSCREENFLAGS_MODE_PAL
...
#define RP_GUESTSCREENFLAGS_HORIZONTAL(m)                                                                                                                                     \
  ((m)&RP_GUESTSCREENFLAGS_HORIZONTALMASK) // returns RP_GUESTSCREENFLAGS_HORIZONTAL_LORES, RP_GUESTSCREENFLAGS_HORIZONTAL_HIRES or RP_GUESTSCREENFLAGS_HORIZONTAL_SUPERHIRES
...
#define RP_IPD_MOUSE1                                                                                                                                                         \
  L"Mouse1" // \0\0-terminated first mouse type ("Mouse1\0\0" = default Windows mouse, or exact mouse described as
            // "Mouse1\\\?\HID#VID_046D&PID_C521&MI_00#8&3b7afb0d&0&0000#{378de44c-56ef-11d1-bc8c-00a0c91405dd\0\0")
#define RP_IPD_JOYSTICK1                                                                                                                                                      \
  L"Joystick1" // \0\0-terminated first joystick type (e.g. standard joystick for WinUAE, described as "Joystick1\0ProductGUID InstanceGUID\0ProductName\0\0"); ProductName
               // must be stripped of trailing spaces, if any
#define RP_IPD_JOYSTICK2                                                                                                                                                      \
  L"Joystick2" // \0\0-terminated second joystick type (e.g. X-Arcade (Left) joystick for WinUAE, described as "Joystick2\0ProductGUID InstanceGUID\0ProductName\0\0");
               // ProductName must be stripped of trailing spaces, if any
#define RP_IPD_JOYSTICK3                                                                                                                                                      \
  L"Joystick3" // \0\0-terminated third joystick type (e.g. X-Arcade (Right) joystick for WinUAE, described as "Joystick3\0ProductGUID InstanceGUID\0ProductName\0\0");
               // ProductName must be stripped of trailing spaces, if any
