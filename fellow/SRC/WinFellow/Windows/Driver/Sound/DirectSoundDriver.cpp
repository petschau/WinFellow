...
void DirectSoundDriver::DSoundFailure(const char *header, HRESULT errorCode)
{
  ...
  Service->Log.AddLog(DSoundErrorString(errorCode));
  Service->Log.AddLog("\n");
}
...
{
  Service->Log.AddLog(description);
  Service->Log.AddLog(predicate ? " - Yes\n" : " - No\n");
}
...
#ifdef _DEBUG
  Service->Log.AddLog("DirectSoundDriver::DSoundSetVolume(): volume %d scaled to %d, setting volume...\n", volume, vol);
#endif
...
bool DirectSoundDriver::InitializeSecondaryBufferNotification()
{
  ...
  else
  {
    ...
    {
      Service->Log.AddLog("DirectSoundDriver::InitializeSecondaryBufferNotification(): timeGetDevCaps() failed\n");
      return false;
    }
    ...
    {
      Service->Log.AddLog("DirectSoundDriver::InitializeSecondaryBufferNotification(): timeBeginPeriod() failed\n");
      return false;
    }
    ...
    {
      Service->Log.AddLog("DirectSoundDriver::InitializeSecondaryBufferNotification(): timeSetEvent() failed\n");
      return false;
    }
