...
static char *wguiGetBOOLEToString(BOOLE value) {
  return (value) ? "yes" : "no";
}
...
INT_PTR CALLBACK wguiPresetDialogProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
  switch (uMsg) 
  {
    case WM_INITDIALOG:
    {
      char *strLastPresetROMDir, strAmigaForeverROMDir[CFG_FILENAME_LENGTH] = "";
...
void wguiSetProcessDPIAwareness(const char *pszAwareness)
{
  ...
  nAwareness = (Process_DPI_Awareness)strtoul(pszAwareness, nullptr, 0);
  hr = E_NOTIMPL;
