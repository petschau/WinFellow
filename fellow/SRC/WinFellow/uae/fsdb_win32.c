...
int fsdb_name_invalid(const char *n)
{
  ...
  {
    if (strchr(n, evilchars[i]) != nullptr) return 1;
  }
...
int fsdb_set_file_attrs(a_inode *aino, int mask)
{
  ...
  if (stat(aino->nname, &statbuf) == -1) return ERROR_OBJECT_NOT_AROUND;
...
int fsdb_mode_representable_p(const a_inode *aino)
{
  ...
  return (aino->amigaos_mode & (A_FIBF_DELETE | A_FIBF_SCRIPT | A_FIBF_PURE | A_FIBF_EXECUTE | A_FIBF_READ | A_FIBF_WRITE)) == 0;
}
