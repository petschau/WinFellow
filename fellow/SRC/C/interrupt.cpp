...
ULO interruptGetPendingChipInterruptNumber()
{
...
ULO interruptGetPendingCpuLevel()
{
...
{
  return scheduler.GetCycleFromCycle280ns(10);
}
...
UWO interruptGetPendingBitMask()
{
...
void interruptRaisePendingInternal(bool delayIRQ)
{
  ...
  if (interruptEvent.IsEnabled())
  {
    ...
    else
    {
      ...
      scheduler.RemoveEvent(&interruptEvent);
      interruptEvent.Disable();
    }
  }
  ...
  for (int interrupt_number = 13; interrupt_number >= 0; interrupt_number--)
  {
    if (interruptIsPending(interrupt_number, pending_chip_interrupts))
    {
      ...
      if (interrupt_level > current_cpu_level)
      {
        ...
        {
          interruptEvent.cycle = scheduler.GetFrameCycle(false) + interruptGetScheduleLatency();
          scheduler.InsertEvent(&interruptEvent);
          return;
        }
...
{
  scheduler.RebaseForNewFrame(&interruptEvent);
}
