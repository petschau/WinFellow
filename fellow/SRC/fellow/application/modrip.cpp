...
#define RIPLOG1(x) Service->Log.AddLog(x);
#define RIPLOG2(x, y) Service->Log.AddLog(x, y);
#define RIPLOG3(x, y, z) Service->Log.AddLog(x, y, z);
#else
#define RIPLOG1(x)
#define RIPLOG2(x)
#define RIPLOG3(x)
...
BOOLE modripSaveMem(struct ModuleInfo *info, MemoryAccessFunc func)
{
  ...
  FILE *modfile = fopen(info->filename, "w+b");
  if (modfile == NULL) return FALSE;
  for (ULO i = info->start; i <= info->end; i++)
...
static void modripDetectProTracker(ULO address, MemoryAccessFunc func)
{
  ...
  {
    if (((*func)(address + 0) == formats[type].ID[0]) && ((*func)(address + 1) == formats[type].ID[1]) && ((*func)(address + 2) == formats[type].ID[2]) &&
        ((*func)(address + 3) == formats[type].ID[3]))
    {
      ...
      if ((info.songlength > MODRIP_MAXMODLEN) || (info.songlength == 0)) return;

      /* scan for max. amount of patterns */
      info.maxpattern = 0;

      for (ULO i = 0; i <= info.songlength; i++)
      {
      ...
      if ((info.end - info.start < MODRIP_MAXMODLEN))
      {
        ...
        if (strlen(info.modname) > 2)
        {
          ...
          for (ULO i = 0; (i < 20) && (info.modname[i] != 0); i++)
          {
...
static void modripDetectSoundFX(ULO address, MemoryAccessFunc func)
{
  ...
  if (((*func)(address + 0) == 'S') && ((*func)(address + 1) == 'O'))
  {
    ...
    if (match)
    {
      ...
      for (ULO i = 0; i < info.instruments; i++)
      ...
      if ((patterns > MODRIP_MAXMODLEN) || (patterns == 0)) return;
      ...
      /* pattern table */
      for (ULO i = 0; i < patterns; i++)
        info.maxpattern = std::max<unsigned int>(info.maxpattern, (*func)(info.start + offset + i));

      if ((info.maxpattern > MODRIP_MAXMODLEN) || (info.maxpattern == 0)) return;
...
static void modripDetectSoundMon(ULO address, MemoryAccessFunc func)
{
  ...
  if (((*func)(address + 0) == 'B') && ((*func)(address + 1) == 'P') && ((*func)(address + 2) == 'S') && ((*func)(address + 3) == 'M'))
  {
    FoundHeader = TRUE;
    modripModuleInfoInitialize(&info);
  ...
  if (((*func)(address + 0) == 'V') && ((*func)(address + 1) == '.'))
  {
    modripModuleInfoInitialize(&info);
    if ((*func)(address + 2) == '2')
    {
  ...
  for (i = 0; i < patterns * 4; i++)
  {
    ...
    info.maxpattern = std::max<unsigned int>(info.maxpattern, temp);
  }
  ...
  if ((info.end - info.start < MODRIP_MAXMODLEN))
  {
    ...
    if (strlen(info.modname) > 2)
    {
      ...
      for (i = 0; (i < 26) && (info.modname[i] != 0); i++)
      {
        ...
        if (isupper(info.modname[i])) info.modname[i] = tolower(info.modname[i]);
      }
      if (!ScratchyName)
      {
...
static void modripDetectFred(ULO address, MemoryAccessFunc func)
{
  ...
  constexpr ULO jmp_68k = 0x4efa;
  constexpr ULO mov_68k = 0x123a;
  constexpr ULO cmp_68k = 0xb0016200;

  /* Fred files start with a jump table */
  if ((BEWORD(address + 0) == jmp_68k) && (BEWORD(address + 4) == jmp_68k) && (BEWORD(address + 8) == jmp_68k) && (BEWORD(address + 12) == jmp_68k))
  {
    ...
    for (i = 0; i < 64; i++)
    {
      if ((BEWORD(address + offset + 0) == mov_68k) && (BEDWORD(address + offset + 4) == cmp_68k))
      {
        match = TRUE;
        break;
      }
  ...
  for (i = 0; (i < 512) && (BEWORD(address + i + 0) != 0x4bfa) && (BEWORD(address + i + 4) != 0xdbfa); i += 2)
    ;
  ...
  for (j = 0; (j < 254) && (BEWORD(address + i + j + 0) != 0x47fa) && (BEWORD(address + i + j + 4) != 0xd7fa); j += 2)
    ;
  ...
  {
    if ((*func)(address + i) == 0x83) instMax = std::max<unsigned int>((*func)(address + i + 1), (ULO)instMax);
  }
  instMax++;
  ...
  for (i = 0; i < (ULO)instMax; i++)
  {
    ...
    if ((BEWORD(address + instData + i * 64 + 4) == 0) && (sampDataOffset != 0) && (sampDataOffset < 0x2ffff))
    {
      sampSize = BEWORD(address + instData + i * 64 + 6);
      sampMax = std::max<long>(sampMax, (sampSize * 2 + sampDataOffset));
      instNo++;
    }
  }
...
static void modripDetectProRunner2(ULO address, MemoryAccessFunc func)
{
  ...
  if ((BYTE(address + 0) != 'S') || (BYTE(address + 1) != 'N') || (BYTE(address + 2) != 'T') || (BYTE(address + 3) != '!')) return;
  ...
  for (ULO i = 0; i < 31; i++)
  {
    if (BYTE(address + 10 + 8 * i) > 0xf) return;
  }
  ...
  for (ULO i = 0; i < 31; i++)
  {
    if (BYTE(address + 11 + 8 * i) > 0x40) return;
  }
...
static void modripDetectThePlayer4(ULO address, MemoryAccessFunc func)
{
  ...
  ULO sampSizeCurr = 0, loopSizeCurr = 0;
  BOOLE match = FALSE;

  if ((BYTE(address + 0) == 'P') && (BYTE(address + 1) == '4'))
  {
...
static void modripScanFellowMemory()
{
  ...
  if (modripGuiRipMemory())
  {
    ...
    if (ChipSize)
    {
      ...
      for (ULO i = 0; i < ChipSize; i++)
        for (ULO j = 0; j < MODRIP_KNOWNFORMATS; j++)
    ...
    if (BogoSize)
    {
      ...
      for (ULO i = 0xc00000; i < (0xc00000 + BogoSize); i++)
        for (ULO j = 0; j < MODRIP_KNOWNFORMATS; j++)
          (*DetectFunctions[j])(i, memoryReadByte);
    }
    ...
    if (FastSize)
    {
      ...
      for (ULO i = 0x200000; i < (0x200000 + FastSize); i++)
        for (ULO j = 0; j < MODRIP_KNOWNFORMATS; j++)
          (*DetectFunctions[j])(i, memoryReadByte);
    }
  }
...
static BOOLE modripReadFloppyImage(char *filename, char *cache)
{
  ...
  if (f != nullptr)
  {
    size_t readbytes = fread(cache, sizeof(char), MODRIP_ADFSIZE, f);
    if (readbytes != MODRIP_ADFSIZE)
    {
      fclose(f);
...
void modripChipDump()
{
  ...
  if (modripGuiDumpChipMem())
  {
    ...
    Service->Fileops.GetGenericFileName(filenamechip, "WinFellow", "chip.mem");
    Saved = modripSaveChipMem(filenamechip);
    if (memoryGetSlowSize())
    {
      char filename[CFG_FILENAME_LENGTH];
      Service->Fileops.GetGenericFileName(filename, "WinFellow", "bogo.mem");
      modripSaveSlowMem(filename);
    }
    if (memoryGetFastSize())
    {
      char filename[CFG_FILENAME_LENGTH];
      Service->Fileops.GetGenericFileName(filename, "WinFellow", "fast.mem");
    ...
    if (Saved)
    {
      if (!access("prowiz.exe", 04))
      {
        ...
        if (modripGuiRunProWiz())
        {
          ...
          Service->Log.AddLog("Running Pro-Wizard: %s ...\n", commandline);
          result = system(commandline);
          Service->Log.AddLog("Pro-Wizard call returned: %d\n", result);
        }
      }
    }
  }
