...
//  if (old_active != gfx_drv_win_active)
...
//        case IDC_ABOUT:
//          MessageBox(hwnd, "Fellow Amiga Emulator V0.4", "Fellow Amiga Emulator", MB_OK);
//	  return 0;
        case SC_SCREENSAVE:
	  return 0;
        default:
...
BOOLE gfxDrvWindowInitialize(gfx_drv_ddraw_device *ddraw_device) {
  if (ddraw_device->mode->windowed) {
    gfx_drv_hwnd = CreateWindowEx(0,
      "FellowWindowClass",
      "Fellow Alpha V0.4 WinNT 991205",
...
void gfxDrvDDrawDeviceInformationDump(void) {
  ...
  sprintf(s,
    "DirectDraw devices found: %d\n",
    listCount(gfx_drv_ddraw_devices));
  fellowAddLog(s);
  for (l = gfx_drv_ddraw_devices; l != NULL; l = listNext(l)) {
    gfx_drv_ddraw_device *tmpdev = (gfx_drv_ddraw_device *) listNode(l);
    sprintf(s,
      "DirectDraw Driver Description: %s\n",
      tmpdev->lpDriverDescription);
    fellowAddLog(s);
    sprintf(s,
      "DirectDraw Driver Name       : %s\n\n",
      tmpdev->lpDriverName);
    fellowAddLog(s);
  }
}
...
void gfxDrvDDrawModeInformationDump(gfx_drv_ddraw_device *ddraw_device) {
  ...
  for (l = ddraw_device->modes; l != NULL; l = listNext(l)) {
    ...
    if (!tmpmode->windowed)
      sprintf(s,
      "Mode Description: %dWx%dHx%dBPPx%dHZ (%d,%d,%d,%d,%d,%d)\n",
      tmpmode->width,
      tmpmode->height,
      tmpmode->depth,
      tmpmode->refresh,
      ...
      sprintf(s,
      "Mode Description: %dWx%dHxWindow\n",
      tmpmode->width,
      tmpmode->height);
    fellowAddLog(s);
  }
...
void gfxDrvDDrawSurfaceBlit(gfx_drv_ddraw_device *ddraw_device) {
  ...
  dstwin.right = ddraw_device->hwnd_clientrect_screen.left + draw_width_amiga_real;
  dstwin.bottom = ddraw_device->hwnd_clientrect_screen.top + draw_width_amiga_real;
  
  /* This can fail when we lose the surface */
  if ((err = IDirectDrawSurface_Blt(ddraw_device->lpDDSPrimary,
...
ULO gfxDrvDDrawSurfacesInitialize(gfx_drv_ddraw_device *ddraw_device) {
  HRESULT err;
  ULO buffercountactual, trybuffers;
  DDSCAPS ddscaps;
  BOOLE success = FALSE;
  
  trybuffers = (ddraw_device->mode->windowed) ? 1 : ddraw_device->maxbuffercount;
  while (trybuffers != 0 && !success) {
    if (ddraw_device->lpDDSPrimary != NULL)
  ...
  if (success) {
	...
	if ((ddraw_device->buffercount != 0) && (ddraw_device->mode->windowed)) {
	...
	fellowAddLog("gfxDrvDDrawSurfacesInitialized(): Window mode - unsupported Pixelformat\n");
	gfxDrvDDrawSurfacesRelease(ddraw_device);
	ddraw_device->buffercount = 0;
      }
    }
  ...
  /* Create a secondary buffer if needed */
  
  if (success && (ddraw_device->mode->windowed || ddraw_device->use_blitter)) {
...
UBY *gfxDrvValidateBufferPointer(void) {
  ...
  if (gfx_drv_displaychange) {
    fellowAddLog("Displaymode change\n");
    soundEmulationStop();
...
void gfxDrvEmulationStart(draw_mode *dm, ULO maxbuffercount) {
  gfx_drv_ddraw_mode *mode;
  ...
  fellowAddLog("End of gfxDrvEmulationStart()\n");
}

ULO gfxDrvEmulationStartPost(void) {
  ULO buffers;

  fellowAddLog("Before call to gfxDrvDDrawSetMode()\n");
  if ((buffers = gfxDrvDDrawSetMode(gfx_drv_ddraw_device_current)) == 0)
    fellowAddLog("gfxDrvEmulationStart(): Zero buffers, gfxDrvDDSetMode() failed\n");
  fellowAddLog("End of call to gfxDrvDDrawSetMode()\n");
  if (gfx_drv_hwnd != NULL)
